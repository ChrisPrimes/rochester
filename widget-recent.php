 ...
 *
 * @package    rochester
 * @author     Rochester Museum of Fine Arts
 * @copyright  Copyright (c) 2022, RMFA
 * @license    http://www.gnu.org/licenses/gpl-2.0.html
 * @since      1.0.0
 */
class Rochester_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			...
			'description' => __( 'Display recent posts without thumbnails.', 'blogsite' )
		);

		// Create the widget.
		parent::__construct(
			'rochester-recent', // $this->id_base
			__( 'Rochester Recent Posts', 'blogsite' ), // $this->name
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'blogsite' ),
			'limit'     => 6,
			'show_number' => true
		);
	...
	?>
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'limit' ) ); ?>">
				<?php esc_html_e( 'Number of posts to show', 'blogsite' ); ?>
			</label>
		...
		<p>
			...
			<label for="<?php echo esc_html( $this->get_field_id( 'show_number' ) ); ?>">
				<?php esc_html_e( 'Display number before post title?', 'blogsite' ); ?>
			</label>
		</p>
